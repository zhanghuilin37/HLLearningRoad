...
-(id)init{
    if (self=[super init]) {
        ...
        _bubbleColor = [UIColor colorWithRed:1 green:0 blue:0 alpha:1];
        _allowPan=YES;
...
-(void)start{
    ...
    self.bubbleLabel.frame = CGRectMake(1, 0, frontView.bounds.size.width-2, frontView.bounds.size.height);
    ...
    [self.superview addSubview:_bombImageView];
    ...
    if (ges.state == UIGestureRecognizerStateBegan) {
        ...
        CGFloat newCritical=sqrtf(xDist*xDist+yDist*yDist);
            ...
            if (r1 <=_breakViscosity&&critical<newCritical) {
                [self hiddenView];
                _bombImageView.center = frontView.center;
